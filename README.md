...
| --- | --- | --- | --- |
| TypeScript | `@bariscemant/verimor` | npm'de `0.2.0` | Node.js >=22.14, ESM |
| Python | `verimor-sdk` / import `verimor` | PyPI'da `0.2.0` | Python >=3.11 |
| Go | `github.com/0Baris/verimor-sdk/packages/go` | `v0.2.0` | Go >=1.24 |
| Dart | `verimor_sms`, `verimor_switch`, `verimor_whatsapp` | Deneysel, pub.dev'de değil | Dart >=2.18 |

Client façade'ları ve generated raw istemciler toplam 68 OpenAPI operasyonunu
kapsar: SMS 13, Switch 52, WhatsApp 3. Tam dil bazlı metot listesi
[`docs/operations.md`](docs/operations.md) içindedir. Generator ve şemaları işleyen
private otomasyon bu repoda bulunmaz; SDK'yı kullanmak için generator'a ihtiyacınız
yoktur.

## Hangi yüzeyi kullanmalıyım?

- Dört dilde de ürün client'ını oluşturup endpoint'i doğrudan metot olarak çağırın.
  Credentials, 30 saniyelik timeout ve HTTP hata normalizasyonunu client yönetir.
- Düşük seviye response/header erişimi veya generated modeller gerektiğinde her
  client'ın `raw` alanını kullanın.
...
const senderIds = await sms.listSenderIds();
...
Go paketi `NewSMSClient`, `NewSwitchClient` ve `NewWhatsAppClient` façade'larıyla
otomatik auth, timeout ve normalize API hatası sunar; generated istemci `Raw`
alanında kalır.
...
TypeScript, Python, Go ve Dart façade çağrılarında 2xx dışındaki yanıtlar
`VerimorApiError` üretir. Hata nesnesinde `product`, `status` ve `body` alanları
bulunur. Ağ, DNS, bağlantı ve timeout hataları dilin native `fetch`, `httpx`,
`error` veya `DioException` sözleşmesiyle korunur. Generated raw istemcilerin kendi
yanıt ve hata sözleşmesi vardır; ayrıntılar dil kılavuzlarında gösterilir.

Dört dildeki façade istemcilerinin varsayılan timeout'u 30 saniyedir. Otomatik
retry veya rate limiter yoktur. `429` ya da geçici sunucu
...
Operasyonların tamamı public repodaki [`facade-manifest.json`](facade-manifest.json)
ve [`docs/operations.md`](docs/operations.md) dosyalarında listelenir.
...
WhatsApp. TypeScript and Python `0.2.0` packages are available on npm and PyPI;
Go is available as a tagged module, while Dart packages remain experimental and
are not published to pub.dev. All 68 operations have first-class client methods
plus generated raw access. All clients have been verified only
